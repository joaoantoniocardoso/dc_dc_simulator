...
class State:
    ...
    def change_state_controller(self, switches):
        ...
        for state in self.__next_state:
            if any(control[0] == "control" for control in state.__control_list) and all(
                ~(state.__index ^ index) & 2 ** (switches[dummy] - 1)
                for dummy in range(len(switches))
            ):
    ...
    def new_time(self, state, time, last_value, independent_sources):
        ...
        """
        sum1 = (
            np.matmul(state.__active_control[1], last_value) - state.__active_control[2]
        )
        if abs(sum1) > self.__absolute_error:
            sum2 = np.matmul(
                state.__active_control[1],
                np.matmul(self.__A, last_value)
                + np.matmul(self.__B, independent_sources),
            )
            sum3 = np.matmul(
                state.__active_control[1],
                np.matmul(self.__A**2, last_value)
                + np.matmul(self.__A, np.matmul(self.__B, independent_sources)),
            )
