...
class System:
    ...
    def get_state_variables_position(self):
        return (
            self.__number_nodes
            + self.__number_voltage_sources
            + self.__number_switches
            - 1
        )
    ...
    def set_control(self, control_switches_state, state):
        ...
        self.__controlled_switches = (
            self.__controlled_switches | control_switches_state
    ...
    def initialize(self):
        ...
        self.__total_number_equations = (
            self.__number_nodes
            + self.__number_inductors
            + self.__number_capacitors
            + self.__number_voltage_sources
            + self.__number_switches
            - 1
        )
        ...
        for element in self.__elements:
            if isinstance(element, Voltage_source):
                ...
                self.__sources_symbols[element.get_index() - 1] = sympify(
                ...
                self.__state_variables_symbols[element.get_index() - 1] = sympify(
                ...
                self.__state_variables_symbols[
                    self.__number_inductors + element.get_index() - 1
                ] = sympify("vc" + str(element.get_index()))
                self.__initial_values[
                    self.__number_inductors + element.get_index() - 1
                ] = element.get_initial_value()
                ...
                element.set_position(
                    self.__number_nodes + self.__number_voltage_sources - 1
                )
                self.__switches.append(element)
            elif isinstance(element, Current_source):
                self.__sources_symbols[
                    self.__number_voltage_sources + element.get_index() - 1
                ] = sympify("I" + str(element.get_index()))
                self.__sources_values[element.get_index() - 1] = element.get_value()
                self.__independent_sources[
                    self.__number_voltage_sources + element.get_index() - 1
                ] = element.get_value()
    ...
    def initialise_output(self):
        ...
        for lst in self.__output_list:
            if lst[0] == "node":
                ...
                self.__output.add_index(
                    self.__number_nodes + self.__number_voltage_sources + lst[1] - 2
                )
