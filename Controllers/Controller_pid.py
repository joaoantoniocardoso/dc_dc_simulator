...
class Controller:
        ...
        if discretization_method == "None":
            ...
            for coefficient in coefficients:
                ...
                self.calculate_coeffs(
                    discretization_method, coefficient[0], coefficient[1]
                )

    def calculate_coeffs(
        self, discretization_method, nominator, denominator
    ):  # calculate coeffs -- used only once inside initialization
        """
        ...
        """
        c_tf_poly_nom = poly1d(
            nominator
        )  # continuous tf nominator list coefficients to poly1d class
        c_tf_poly_denom = poly1d(
            denominator
        )  # continuous tf denominator list coefficients to poly1d class
        ...
        # match gain:
        K = (
            abs(d_tf_poly_denom(exp(0.01 * self.__period)))
            * abs(c_tf_poly_nom(0.01j))
            / (
                abs(d_tf_poly_nom(exp(0.01 * self.__period)))
                * abs((c_tf_poly_denom(0.01j)))
            )
        )
        ...
        for i in range(d_tf_poly_denom.order):
            duty_ratio_coef.append(
                -d_tf_poly_denom[d_tf_poly_denom.order - 1 - i]
                / d_tf_poly_denom[d_tf_poly_denom.order]
            )
        ...
        for i in range(d_tf_poly_nom.order + 1):
            coeffs.append(
                d_tf_poly_nom[d_tf_poly_nom.order - i]
                / d_tf_poly_denom[d_tf_poly_denom.order]
            )
    ...
    def check_change(self, time, step):  # for now easy check for duty ratio
        ...
        """
        if (
            (time - floor(time / self.__period) * self.__period)
            <= (self.__duty_ratio[-1] * self.__period)
            and (time - floor(time / self.__period) * self.__period + step)
            > (self.__duty_ratio[-1] * self.__period)
        ) or (
            ((time - floor(time / self.__period) * self.__period) <= self.__period)
            and (
                (time - floor(time / self.__period) * self.__period + step)
                > self.__period
            )
        ):
            return True
    ...
    def is_period(self, time, step):
        return round(time, 9) == round(
            round(time * self.__frequency, 0) / self.__frequency, 9
        )
    ...
    def change_time(self, time):
        ...
        """
        if (time - floor(time / self.__period) * self.__period) <= (
            self.__duty_ratio[-1] * self.__period
        ):
            return (
                self.__duty_ratio[-1] * self.__period
                + floor(time / self.__period) * self.__period
            )
        else:
    ...
    def calculate_current_duty_ratio(self):
        for dummy in range(len(self.__error_list)):
            self.__error_list[dummy].append(
                -self.__current_output[dummy] + self.__reference_values[dummy]
            )
        ...
        for dummy_i in range(len(self.__error_list)):
            ...
            for dummy_j in range(
                min(
                    len(self.__error_coeffs[dummy_i]),
                    len(self.__error_list[dummy_i])
                    - self.__error_leading_param[dummy_i],
                )
            ):
                result += (
                    self.__error_list[dummy_i][
                        -1 - self.__error_leading_param[dummy_i] - dummy_j
                    ]
    ...
    def printing(self):
        ...
        for dummy in range(len(self.__error_list)):
            f.write(
                "leading index / duty ratio coeffs / error coeffs "
                + str(self.__error_leading_param[dummy])
                + " "
                + str(self.__duty_ratio_coeffs[dummy])
                + " "
        ...
        f.write(
            "result calculation -- previous duty ratio limit: " + str(self.__result)
        )
