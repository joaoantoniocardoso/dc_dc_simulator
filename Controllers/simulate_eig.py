...
def simulate(
    beginning_state, system, controller, simulation_time, TIME_STEP_POINTS, file_name
):
    print
    ...
    state_variables = np.array(
        [
            [0 for dummy in range(COUNTER)]
            for dummy in range(system.get_number_state_variables())
        ],
        dtype=float,
    )
    output = np.array(
        [
            [0 for dummy in range(COUNTER)]
            for dummy in range(system.get_number_outputs())
        ],
        dtype=float,
    )
    ...
    output[:, 0] = np.matmul(matrices[2], state_variables[:, counter]) + np.matmul(
        matrices[3], independent_sources
    )
    ...
    while time[counter] <= simulation_time:
        ...
        if counter >= COUNTER - 1:
            if time[0] > 0:
                np.save(
                    npy_file,
                    np.row_stack((time[1:counter], state_variables[:, 1:counter])),
                )
            else:
                np.save(
                    npy_file,
                    np.row_stack((time[0:counter], state_variables[:, 0:counter])),
                )
            state_variables[:, 0] = state_variables[:, counter]
            time[0] = time[counter]
            counter = 0
            print(
                "Part of data stored. Elpased " + str(datetime.datetime.now() - start)
            )

        # checking for the state change
        if counter > 0:
            help_state = state.change_state_internally(
                state_variables[:, counter - 1 : counter + 1]
            )
        else:
            help_state = state.change_state_internally(
                state_variables[:, counter - 2 : COUNTER]
            )
        ...
        if help_state != None:
                ...
                if counter >= COUNTER - 1:
                    if time[0] > 0:
                        np.save(
                            npy_file,
                            np.row_stack(
                                (time[1:counter], state_variables[:, 1:counter])
                            ),
                        )
                    else:
                        np.save(
                            npy_file,
                            np.row_stack(
                                (time[0:counter], state_variables[:, 0:counter])
                            ),
                        )
                    state_variables[:, 0] = state_variables[:, counter]
                    time[0] = time[counter]
            ...
            if controller.is_period(time[counter], step):
                ...
                if controller.get_duty_ratio() != 0:
                    ...
                    state = beginning_state.change_state_controller(
                        controller.get_switches()
                    )
            else:
                state = state.change_state_controller(controller.get_switches())
    ...
    if counter > 0:
        if time[0] > 0:
            np.save(
                npy_file, np.row_stack((time[1:counter], state_variables[:, 1:counter]))
            )
        else:
            np.save(
                npy_file, np.row_stack((time[0:counter], state_variables[:, 0:counter]))
            )
...
def get_particular_solution(last_value, matrix_A, matrix_B):
    ...
    #        pass
    particular_solution = -np.matmul(
        np.matmul(np.linalg.pinv(matrix_A), matrix_B), independent_sources
    )
...
def get_homogeneous_solution_coeffs(time, last_value, matrix_A, matrix_B):
    ...
    M = np.array(
        [
            [0 for dummy in range(size_A * number_eigvals)]
            for dummy in range(size_A * number_eigvals)
        ],
        dtype=complex,
    )
    ...
    for dummy_i in range(number_eigvals):
        for dummy_j in range(0, len(eigenvalues)):
            M[
                dummy_i * size_A : (dummy_i + 1) * size_A,
                dummy_j * size_A : (dummy_j + 1) * size_A,
            ] = (
                np.eye(size_A)
                * eigenvalues[dummy_j] ** dummy_i
                * np.exp(eigenvalues[dummy_j] * time)
            )

    x[:size_A] = last_value - particular_solution
    for dummy in range(1, number_eigvals):
        x[dummy * size_A : (dummy + 1) * size_A] = np.matmul(
            np.linalg.matrix_power(matrix_A, dummy), last_value
        ) + np.matmul(
            np.linalg.matrix_power(matrix_A, dummy - 1),
            np.matmul(matrix_B, independent_sources),
        )
...
def solve_step(time, counter):
    ...
    M = np.array(
        [[0 for dummy in range(num_eig * size_A)] for dummy in range(size_A)],
        dtype=complex,
    )
    for dummy in range(num_eig):
        M[:, dummy * size_A : (dummy + 1) * size_A] = np.eye(size_A) * np.exp(
            eigenvalues[dummy] * time
        )

    solution = np.matmul(M, coeffs) + particular_solution

    state_variables[:, counter] = solution.real
    output[:, counter] = np.matmul(matrices[2], solution.real) + np.matmul(
        matrices[3], independent_sources
    )
...
def get_state_parameters(state, counter):
    ...
    particular_solution = get_particular_solution(
        state_variables[:, counter], matrices[0], matrices[1]
    )
    coeffs = get_homogeneous_solution_coeffs(
        time[counter], state_variables[:, counter], matrices[0], matrices[1]
    )
