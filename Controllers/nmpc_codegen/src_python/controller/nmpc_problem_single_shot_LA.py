...
class Single_shot_LA_definition:
    ...
    def generate_cost_general_constraints(
        self, current_state, input, lambdas, general_constraint_weights, step_horizon
    ):
        """
        ...
        """
        offset_constraints = (
            step_horizon * self._controller.number_of_general_constraints
        )
        cost = cd.SX(1, 1)
        for i in range(0, self._controller.number_of_general_constraints):
            constraint_cost = self._controller.general_constraints[i].evaluate_cost(
                current_state, input
            )
            ...
            cost = (
                cost
                + (constraint_cost**2)
                * general_constraint_weights[offset_constraints + i]
            )
    ...
    def generate_cost_function(self):
        ...
        Generate Casadi code of cost and gradient function:
                ...
                - Casadi functions
        """
        initial_state = cd.SX.sym(
            "initial_state", self._controller.model.number_of_states, 1
        )
        state_reference = cd.SX.sym(
            "state_reference", self._controller.model.number_of_states, 1
        )
        input_reference = cd.SX.sym(
            "input_reference", self._controller.model.number_of_inputs, 1
        )

        lambdas = cd.SX.sym(
            "lambdas",
            self._controller.number_of_general_constraints * self._controller.horizon,
            1,
        )
        general_constraint_weights = cd.SX.sym(
        ...
        constraint_weights = cd.SX.sym(
            "constraint_weights", self._controller.number_of_constraints, 1
        )
        ...
        for i in range(1, self._controller.horizon + 1):
            ...
            step_horizon = i - 1
            general_constraints_cost = self.generate_cost_general_constraints(
                current_state, input, lambdas, general_constraint_weights, step_horizon
            )
        ...
        ) = ccg.setup_casadi_functions_and_generate_c(
        ...
        for i in range(0, self._controller.horizon):
            input = input_all_steps[
                i
                * self._controller.model.number_of_inputs : (i + 1)
                * self._controller.model.number_of_inputs
            ]
            ...
            constraint_values = self.evaluate_constraints(
                state, input, constraint_values, i
            )

        ccg.generate_c_constraints(
            initial_state, input_all_steps, constraint_values, self._controller.location
        )
