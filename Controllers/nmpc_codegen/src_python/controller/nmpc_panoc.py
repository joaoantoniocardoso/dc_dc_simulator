...
class Nmpc_panoc:
    ...
    def __init__(self, location_lib, model, stage_cost, terminal_cost=None):
        ...
        self._model = model
        self._dimension_panoc = (
            0  # dimension of the panoc problem, should be set so something non-zero
        )
        ...
        self._globals_generator = Globals_generator(
            self._location_lib + "/globals/globals_dyn.h"
        )
    ...
    def generate_code(self):
        ...
        if self._shooting_mode == "single shot":
            ...
            print(
                "ERROR in generating code: invalid choice of shooting mode [single shot|multiple shot]"
            )
    ...
    def __generate_integrator(self):
        ...
        integrator = cd.Function(
            "integrator", [state, input], [self._model.get_next_state(state, input)]
        )
    ...
    def __generate_cost_function_singleshot(self):
        ...
        (
            self._cost_function,
            self._cost_function_derivative_combined,
        ) = ssd.generate_cost_function()
    ...
    def stage_cost(self, current_state, input, i, state_reference, input_reference):
        if i == self._horizon - 1:
            return self._terminal_cost.evaluate_cost(
                current_state, input, i, state_reference, input_reference
            )
        return self._stage_cost.evaluate_cost(
            current_state, input, i, state_reference, input_reference
        )
    ...
    def generate_cost_constraints(self, state, input, state_index, constraint_weights):
        ...
        if self.number_of_constraints is 0:
            ...
            for i in range(0, self.number_of_constraints):
                cost += constraint_weights[i] * self._constraints[i].evaluate_cost(
