...
class Casadi_code_generator:
    @staticmethod
    def setup_casadi_functions_and_generate_c(
        static_casadi_parameters, input_all_steps, obstacle_weights, cost, location_lib
    ):

        cost_function = cd.Function(
            "cost_function",
            [static_casadi_parameters, input_all_steps, obstacle_weights],
            [cost],
        )
        cost_function_derivative_combined = cd.Function(
            "cost_function_derivative_combined",
            [static_casadi_parameters, input_all_steps, obstacle_weights],
            [cost, cd.gradient(cost, input_all_steps)],
        )

        Casadi_code_generator.translate_casadi_to_c(
            cost_function, location_lib, filename="cost_function"
        )
        Casadi_code_generator.translate_casadi_to_c(
            cost_function_derivative_combined,
            location_lib,
            filename="cost_function_derivative_combined",
        )

        return (cost_function, cost_function_derivative_combined)

    @staticmethod
    def generate_c_constraints(
        initial_state, input_all_steps, constraint_values, location_lib
    ):
        constraints_function = cd.Function(
            "evaluate_constraints",
            [initial_state, input_all_steps],
            [constraint_values],
        )
        Casadi_code_generator.translate_casadi_to_c(
            constraints_function, location_lib, filename="evaluate_constraints"
        )
