...
class Globals_generator:
    ...
    def generate_globals(self, nmpc_controller):
        ...
        if nmpc_controller.shift_input:
            self._define_variable("SHIFT_INPUT", str(1.0))
        ...
        if len(nmpc_controller.general_constraints) > 0:
            ...
            self._define_variable(
                "NUMBER_OF_GENERAL_CONSTRAINTS_PER_STEP",
                str(len(nmpc_controller.general_constraints)),
            )
            self._define_variable(
                "CONSTRAINT_OPTIMAL_VALUE",
                str(nmpc_controller.constraint_optimal_value),
            )
            self._define_variable(
                "CONSTRAINT_MAX_WEIGHT", str(nmpc_controller.constraint_max_weight)
            )
        ...
        self._define_variable(
            "NUMBER_OF_CONSTRAINTS", nmpc_controller.number_of_constraints
        )
        ...
        self._define_variable(
            "MIN_RESIDUAL", "(1e" + str(nmpc_controller.min_residual) + ")"
        )
