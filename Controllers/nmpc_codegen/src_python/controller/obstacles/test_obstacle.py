...
class TestObstaclesSquare(unittest.TestCase):
    def setUp(self):
        ...
        h_0 = lambda x: x[1] - 2.0 * math.sin(-x[0] / 2.0)
        h_1 = lambda x: 3.0 * math.sin(x[0] / 2.0 - 1.0) - x[1]
        h_2 = lambda x: x[0] - 1.0
        h_3 = lambda x: 8.0 - x[0]
    ...
    def test_zero_outside_square(self):
        ...
        for i in range(0, 4):
            ...
            self.assertEqual(
                cost[0, 0],
                0.0,
                "error in testpoint " + str(i) + " --> " + str(test_points[:, i]),
            )

    def test_zero_insideside_square(self):
        test_points = np.array([[1.1, 1.5, 1.9, 1.5], [0, 0.9, 0, -0.9]])
        ...
        for i in range(0, 4):
            ...
            self.assertGreater(
                cost[0, 0],
                0.0,
                "error in testpoint i=" + str(i) + " --> " + str(test_points[:, i]),
            )
    ...
    def test_zero_insideside_circle(self):
        test_points = np.array([[0, 0.5, -0.5, -0.05, 0.5], [0, 0.5, -0.5, 0.5, -0.5]])
    ...
    def test_zero_outside_costum(self):
        test_points = np.array([[0.0, 0.0, 1.0, -1.0], [-0.25, 1.25, 0.5, 0.5]])
    ...
    def test_zero_insideside_costum(self):
        test_points = np.array([[-0.5, 0.5, 0.0], [0.5, 0.5, 0.5]])
    ...
    def test_zero_outside_costum2(self):
        test_points = np.array([[2.0, 6.0, 7.0, 7.0], [2.0, -1.0, 3.0, -1.0]])
    ...
    def test_zero_insideside_costum2(self):
        test_points = np.array([[4.0, 5.0, 3.0], [1.0, 1.0, -1.0]])
