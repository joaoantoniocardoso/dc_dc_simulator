...
class Circular(Obstacle):
    ...
    def evaluate_coordinate_state_cost(self, coordinates_state):
        ...
        return Obstacle.trim_and_square(
            self._radius
            - cd.sqrt(cd.sum1((self._center_coordinates - coordinates_state) ** 2))
        )

    def plot(self):
        circle = plt.patches.Circle(
            (self._center_coordinates[0], self._center_coordinates[1]),
            radius=self._radius,
            fill=False,
        )
        ax = plt.pyplot.gca()
