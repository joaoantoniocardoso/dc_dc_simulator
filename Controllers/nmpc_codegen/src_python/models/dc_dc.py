...
class DC_DC:
    def __init__(
        self, A, b, Ad, bd, c, state_reference, input_reference, number_subsystems
    ):
    ...
    def system_equations_period(self, x, u):
        x_dot = (
            cd.mtimes(self._Ad, x - self._state_reference)
            + cd.mtimes(self._bd, u - self._input_reference)
            + self._cd
        )
        return x_dot
...
def get_model(A, b, Ad, bd, c, state_reference, input_reference, number_subsystems):
    ...
    system_equations = lambda state, input, index: model.system_equation(
        state, input, index
    )
    system_equations_period = lambda state, input: model.system_equations_period(
        state, input
    )
