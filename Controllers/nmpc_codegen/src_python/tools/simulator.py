...
class Simulation_data:
    ...
    def time_string(self):
        return (
            ...
            + str(self._seconds)
            + "  "
            + str(self._milli_seconds)
            + ":"
            + str(self._micro_seconds)
            + ":"
            + str(self._nano_seconds)
        )
...
class Simulator:
    ...
    def simulate_nmpc(self, current_state, state_reference, input_reference):
        ...
        self.nmpc_python_interface.simulate_nmpc_panoc.restype = ctypes.POINTER(
            Panoc_time
        )
        ...
        convergence_time = self.nmpc_python_interface.simulate_nmpc_panoc(
            current_state_p, state_reference_p, input_reference_p, optimal_input
        )

        return Simulation_data(convergence_time[0], optimal_input)

    def simulate_nmpc_multistep_solution(
        self, current_state, state_reference, input_reference, dimension_solution
    ):
        """
        Simulate the controller for 1 step
    ...
    def set_init_value_solver(self, value, index):
        ...
        self.nmpc_python_interface.simulation_set_buffer_solution(
            value_ctype, index_ctype
        )
    ...
    def set_weight_constraint(self, index_constraint, weight_constraint):
        ...
        self.nmpc_python_interface.simulation_set_weight_constraints(
            index_constraint_ctype, weight_constraint_ctype
        )
    ...
    def _load_library(self):
        ...
        try:
            if platform.system() == "Linux":
                ...
                extension_lib = ".so"
                lib_location = (
                    self._nmpc_controller_location
                    + "/build/libpython_interface"
                    + extension_lib
                )
                ...
                if self._visual_studio:
                    ...
                    lib_location = (
                        self._nmpc_controller_location
                        + "/build/Release/libpython_interface"
                        + extension_lib
                    )
                    print(
                        "Compiling python interface for Windows using Visual Studio toolset: "
                        + lib_location
                    )
                else:
                    extension_lib = ".dll"
                    ...
                    print(
                        "Compiling python interface for Windows using MINGW: "
                        + lib_location
                    )
            ...
            print(
                "Failed to load the dll, are you sure python and the toolchain are compatible?"
            )
